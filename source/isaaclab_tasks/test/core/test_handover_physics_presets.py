# Copyright (c) 2026, The Isaac Lab Project Developers (https://github.com/isaac-sim/IsaacLab/blob/main/CONTRIBUTORS.md).
# All rights reserved.
#
# SPDX-License-Identifier: BSD-3-Clause

"""Tests for the Shadow Hand handover physics presets."""

from isaaclab_physx.physics import PhysxCfg

from isaaclab_tasks.core.handover.handover_env_cfg import BALL_CFG, HandoverEnvCfg
from isaaclab_tasks.utils.hydra import resolve_presets

from isaaclab_assets.robots.shadow_hand import ShadowHand


def test_handover_isaacsim_physx_resolves_physx_assets() -> None:
    """The concrete PhysX selector must resolve the scene, hands, and object together."""
    env_cfg = resolve_presets(HandoverEnvCfg(), selected=("isaacsim_physx",))

    assert isinstance(env_cfg.sim.physics, PhysxCfg)
    assert env_cfg.right_robot_cfg.spawn.usd_path == ShadowHand.cfg("physx").spawn.usd_path
    assert env_cfg.left_robot_cfg.spawn.usd_path == ShadowHand.cfg("physx").spawn.usd_path
    assert env_cfg.object_cfg == BALL_CFG
