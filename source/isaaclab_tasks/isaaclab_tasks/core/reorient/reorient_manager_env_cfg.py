...
class ReorientObjectEnvCfg(ManagerBasedRLEnvCfg):
    ...
    # Simulation settings. ``sim.physics`` is left unset: the per-backend solver
    # configuration carries robot-specific buffer sizes, so each robot's configuration
    # assigns its own :class:`~isaaclab_tasks.utils.PresetCfg` in ``__post_init__``.
